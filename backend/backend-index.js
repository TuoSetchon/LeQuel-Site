...
async function resolveAndStorePaymentStatus(transactionId) {
  ...
  // Un statut final (paid/failed) ne doit jamais être écrasé par un "pending" tardif
  // (ex: deux vérifications quasi simultanées qui se chevauchent).
  const { rows } = await pool.query(
    `UPDATE lequel_orders SET payment_status=$1
     WHERE transaction_id=$2 AND payment_status NOT IN ('paid','failed')
     RETURNING payment_status`,
    [resolved, transactionId]
  );
  if (rows[0]) return rows[0].payment_status;
  const existing = await pool.query('SELECT payment_status FROM lequel_orders WHERE transaction_id=$1', [transactionId]);
  return existing.rows[0] ? existing.rows[0].payment_status : resolved;
}
