...
// Render a parfois du mal à résoudre certains domaines avec son résolveur DNS par défaut.
// On force les requêtes sortantes (CinetPay) à utiliser des résolveurs publics fiables.
const reliableResolver = new dns.Resolver();
reliableResolver.setServers(['8.8.8.8', '1.1.1.1', '8.8.4.4']);
function reliableLookup(hostname, options, callback) {
  reliableResolver.resolve4(hostname, (err, addresses) => {
    if (err || !addresses || !addresses.length) {
      return dns.lookup(hostname, options, callback); // repli sur le résolveur normal
    }
    callback(null, addresses[0], 4);
  });
}
const reliableHttpsAgent = new https.Agent({ lookup: reliableLookup });
axios.defaults.httpsAgent = reliableHttpsAgent;
...
// CinetPay a parfois des ratés réseau ponctuels (DNS) — on retente automatiquement.
...
async function postWithRetry(url, payload, retries = 2) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.warn(`CinetPay: tentative ${attempt} échouée (${err.code}), nouvel essai...`);
...
app.post('/api/payment/init', async (req, res) => {
  try {
    ...
    const response = await postWithRetry('https://api-checkout.cinetpay.com/v2/payment', payload);
...
async function resolveAndStorePaymentStatus(transactionId) {
  const checkResp = await postWithRetry('https://api-checkout.cinetpay.com/v2/payment/check', {
    apikey: CINETPAY_API_KEY,
    site_id: CINETPAY_SITE_ID,
    transaction_id: transactionId,
  });
